...
public class NettySSLSupport {
  ...
  public static SSLContext createServerContext(final String keystorePath,
    ...
    KeyManager[] keyManagers = NettySSLSupport.loadKeyManagers(keystorePath, keystorePassword);
    TrustManager[] trustManagers = NettySSLSupport.loadTrustManager(false, trustStorePath, trustStorePassword);
  ...
  public static SSLContext createClientContext(final String keystorePath, final String keystorePassword) throws Exception {
    ...
    KeyManager[] keyManagers = NettySSLSupport.loadKeyManagers(keystorePath, keystorePassword);
    TrustManager[] trustManagers = NettySSLSupport.loadTrustManager(true, null, null);
  ...
  public static SSLContext getInstance(final boolean client,
    ...
    if (client) {
      return NettySSLSupport.createClientContext(keystorePath, keystorePassword);
    } else {
      return NettySSLSupport.createServerContext(keystorePath, keystorePassword, trustStorePath, trustStorePassword);
    }
  }
  ...
  private static TrustManager[] loadTrustManager(final boolean clientMode,
    ...
    if (clientMode) {
      ...
      KeyStore trustStore = NettySSLSupport.loadKeystore(trustStorePath, trustStorePassword);
  ...
  private static KeyStore loadKeystore(final String keystorePath, final String keystorePassword) throws Exception {
    ...
    InputStream in = null;
    try {
      URL keystoreURL = NettySSLSupport.validateStoreURL(keystorePath);
  ...
  private static KeyManager[] loadKeyManagers(final String keystorePath, final String keystorePassword) throws Exception {
    ...
    KeyStore ks = NettySSLSupport.loadKeystore(keystorePath, keystorePassword);
